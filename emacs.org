...
(use-package outline
  ...
  (polymode-init-inner . (lambda () (outline-minor-mode 0)))
  :config
  (defun zw/init-outline ()
    (let ((comment-start-symbol (or (string-trim comment-start) "#")))
      (setq outline-level 'outline-level
            outline-regexp
            (rx-to-string `(group (+ ,comment-start-symbol) (+ space) (+ "*")))))
    (outline-minor-mode 1)
    ;; FIXME: Not sure why level 3 shows the first heading
    (outline--show-headings-up-to-level 3))
  (defun zw/outline-tab ()
    (interactive)
