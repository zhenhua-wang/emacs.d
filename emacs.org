...
(defun company-capf-with-R-objects (command &optional arg &rest ignored)
  ...
  (let* ((ess-prefix (company-R-objects--prefix))
         (ess-r-object-p (company-capf-with-R-objects--check-prefix ess-prefix)))
    (cl-case command
      (interactive (company-begin-backend 'company-R-objects))
      (prefix ess-prefix)
      (candidates (if ess-r-object-p
                      (company-R-objects--candidates arg)
                    (company-capf command arg)))
      (annotation (if ess-r-object-p
                      "R-object"
                    (company-capf command arg)))
      (kind (if ess-r-object-p
                'field
              (company-capf command arg)))
