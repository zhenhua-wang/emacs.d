...
(add-hook 'dired-mode-hook
          (lambda ()
            (interactive)
            (setq buffer-face-mode-face
                  (list ':family zw/font-variable-pitch
                        ':height (+ zw/font-size-variable-pitch 50)))
            (buffer-face-mode)
...
(defun zw/modeline-active-inactive-face (face)
 ...
 (list
  ...
  '(:eval
    (propertize
     ...
     'face (zw/modeline-active-inactive-face
  ...
  '(:eval (if (file-remote-p default-directory)
              (propertize "Remote" 'face
                          (zw/modeline-active-inactive-face 'highlight))))
    ...
    (propertize
     ...
     `((space :align-to
              (- (+ right right-fringe right-margin)
                 ,(+ 4 (apply '+ (list (zw/string-width (format-mode-line mode-name))
                                       (zw/string-width (zw/modeline-misc-info))
                                       ))))))))

  ;; misc info
  '(:eval (zw/modeline-misc-info))

  ;; the current major mode for the buffer.
  " ["

  '(:eval (propertize (format-mode-line mode-name) 'face
                      (zw/modeline-active-inactive-face
                       'font-lock-keyword-face)
...
Similar to `marginalia-annotate-symbol', but does not show symbol class."
    (when-let* ((sym (intern-soft cand)))
      (concat
       (let* ((ind (lookup-minor-mode-from-indicator cand))
...
(use-package company-posframe
  ...
  (setq company-posframe-quickhelp-delay nil
        ...
        ;; FIXME: flicker in macos, disable it for now
        ...
        (list :override-parameters
              ...
              :border-color (face-background 'highlight)
              :border-width 3))
...
(defun company-capf-with-R-objects (command &optional arg &rest ignored)
  ...
  (cl-case command
    ...
    (candidates (if (company-capf-with-R-objects--check-prefix
                     (company-R-objects--prefix))
                    (company-R-objects--candidates arg)
                  (company-capf command arg)))
    (annotation (if (company-capf-with-R-objects--check-prefix
                     (company-R-objects--prefix))
                    "R-object"
                  (company-capf command arg)))
    (kind (if (company-capf-with-R-objects--check-prefix
               (company-R-objects--prefix))
              'field
            (company-capf command arg)))
...
(dolist (mode '(prog-mode-hook minibuffer-setup-hook))
  (add-hook mode
            (lambda ()
