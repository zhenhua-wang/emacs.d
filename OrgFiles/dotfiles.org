...
font-2 = "JetBrainsMono Nerd Font:style=Regular:size=13;6"

module-margin-left = 0
module-margin-right = 0
modules-left = powermenu sep sep sep-color emacs-buffer-path emacs-buffer-name corner-right
modules-center = date
...
type = custom/ipc
hook-0 = emacsclient -e "(exwm/polybar-buffer-name)" | sed -e 's/^"//' -e 's/"$//' | awk '{print ""$0""}'
initial = 1
format = "<output> "
...
type = custom/ipc
hook-0 = emacsclient -e "(exwm/polybar-keycast-key)"  | sed -e 's/^"//' -e 's/"$//' | awk '{print ""$0""}'
initial = 1
format = <output>
...
type = custom/ipc
hook-0 = emacsclient -e "(exwm/polybar-keycast-desc)"  | sed -e 's/^"//' -e 's/"$//' | awk '{print ""$0""}'
initial = 1
