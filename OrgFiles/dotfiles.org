...
* Chinese input
#+begin_src sh :tangle (when (eq system-type 'gnu/linux) "~/.config/plasma-workspace/env/start-ibus.sh")
...
* Zathura
#+begin_src conf :tangle (when (eq system-type 'gnu/linux) "~/.config/zathura/zathurarc")
