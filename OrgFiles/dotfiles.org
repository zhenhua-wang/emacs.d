...
modules-center = date
modules-right = emacs-keycast-key sep emacs-keycast-desc sep-long wired-network sep wireless-network sep battery sep tray
separator = ""
...
click-left = bash ~/.config/polybar/systray.sh
...
** system tray menu
#+HEADER: :tangle (if (eq system-type 'gnu/linux) "~/.config/polybar/systray.sh" "no")
#+begin_src sh
#!/bin/bash
...
then
    ...
    stalonetray --background "#1f2024" --window-type normal --icon-size 40 --geometry 1x1-50+0 --grow-gravity E
fi
#+end_src
