...
# compile emacs
yay -S libxft libxpm libtiff giflib aspell aspell-en
git clone git://git.sv.gnu.org/emacs.git
cd emacs
./autogen.sh
./configure --with-json --with-xinput2 --without-compress-install --with-x-toolkit=no
make
sudo make install
#+end_src
...
yay -S firefox kitty htop neofetch obs-studio thunar tumbler ffmpegthumbnailer mpv yt-dlp streamlink file-roller
...
*** GPU and Tensorflow
Make sure that the nvidia driver is the proprietary NVIDIA driver (Manjaro can install this easily). ~Unfortunately~, emacs cannot see cuda. You need to run tensorflow scripts from terminal.
#+begin_src sh
