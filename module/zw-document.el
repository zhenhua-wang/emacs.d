...
(defun zw/open-agenda-dashboard ()
  ...
  (let ((first-org-file (car org-agenda-files)))
    (when (and (not (file-exists-p first-org-file))
               (y-or-n-p (format "%s doesn't exist! Initialize? " first-org-file)))
...
(with-eval-after-load "ob"
  ...
  (zw/merge-and-mutate-symbol-lists
...
(use-package polymode
  ...
  (zw/polymode-disable-advice #'eglot-ensure))
