...
battery_path = next(
    (dev
     ...
     if dev.is_dir()
     and dev.name.startswith("BAT")
     and (dev / "type").is_file()
     and (dev / "type").read_text().strip() == "Battery"),
    None
)
...
def _draw_right_status(screen: Screen, is_last: bool) -> int:
    ...
    battery_percent, battery_status = get_battery_cells()
    if not is_last:
    ...
    if battery_percent is not None:
        if battery_status == "Charging":
            BATTERY_FG = as_rgb(int(to_color("#000000")))
            BATTERY_BG = as_rgb(int(to_color("#98c379")))
            battery_percent = f"󰂄 {battery_percent}"
        elif battery_percent <= 30:
            BATTERY_FG = as_rgb(int(to_color("#000000")))
            BATTERY_BG = as_rgb(int(to_color("#e06c75")))
            battery_percent = f"󰂃 {battery_percent}"
        else:
            BATTERY_FG = as_rgb(color_as_int(opts.active_tab_foreground))
            BATTERY_BG = as_rgb(color_as_int(opts.active_tab_background))
        cells.append((BATTERY_FG, BATTERY_BG, f" {battery_percent}% "))
