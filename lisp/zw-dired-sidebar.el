(defun zw/toggle-dired-sidebar ()
  ...
  (let* ((dir (abbreviate-file-name
         ...
         (sidebar-buffer (if (eq (current-buffer) buffer)
                             (ignore-errors (clone-buffer))
                           buffer)))
    (with-current-buffer sidebar-buffer (zw-dired-sidebar-mode 1))
    (display-buffer-in-side-window
     sidebar-buffer `((side . left) (slot . 0)
                      (window-width . 0.2)
                      (preserve-size . (t . nil))
